...
`/posts` route, and a sign-in page nobody would ship.
...
**Confirm they mean it.** This deletes working code, including the only sign-in
page in the repo (`app/login/` and `features/auth/` are a bare
email-and-password form built to exercise the example, not something a real
project would ship). If the project has no auth UI of its own yet, say so —
losing the ability to sign in is a surprising side effect of "delete the post
domain", and it is better raised now than discovered later.
...
It deletes the seven paths, then makes the eight follow-up edits: the four the
...
the four it would not (comments in `lib/orpc-query.ts`, `lib/orpc.server.ts`,
`api/src/testing/index.ts`, and the note `setup-project` leaves in `CLAUDE.md`).
It also cuts S14 from `docs/architecture.md`, and takes the appendix divider
with it if S14 was the last one standing.
...
four of the eight edits and `rls-guard.test.ts` covers the pinned table list.
Green means the removal is actually complete.
